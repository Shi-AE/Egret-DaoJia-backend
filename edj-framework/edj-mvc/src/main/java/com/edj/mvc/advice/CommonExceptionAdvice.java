...
public class CommonExceptionAdvice {
    ...
    public Result<List<String>> handleValidatedException(Exception e) {
        if (e instanceof MethodArgumentNotValidException) {
            ...
            List<String> collect = ((MethodArgumentNotValidException) e)
                    .getBindingResult()
                    .getAllErrors()
                    .stream()
                    .map(DefaultMessageSourceResolvable::getDefaultMessage)
                    .toList();
            return Result.error("入参校验失败", collect);
        }
        if (e instanceof ConstraintViolationException) {
            // BeanValidation GET simple param
            List<String> collect = ((ConstraintViolationException) e)
        ...
        if (e instanceof BindException) {
            ...
            List<String> collect = ((BindException) e)
                    .getAllErrors()
                    .stream()
                    .map(DefaultMessageSourceResolvable::getDefaultMessage)
                    .toList();
        ...
        return Result.error("入参校验异常捕获失败");
    }
