...
import com.edj.cache.constants.RedisSyncQueueConstants;
...
public class HashSyncThread extends AbstractSyncThread<Object> {
    ...
    protected List<SyncMessage<Object>> getData() {
        ...
        ScanOptions scanOptions = ScanOptions.scanOptions()
                .count(perCount)
                .build();
        try (Cursor<Map.Entry<String, Object>> cursor = redisTemplate.opsForHash().scan(RedisSyncQueueUtils.getQueueRedisKey(getQueueName(), getIndex()), scanOptions)) {
            // scan获取数据
